...
if archivo_crudo_T:
    ...
    if all(col in archivo_excel.columns for col in ['Cuenca', 'Estación']):
        ...
        ruta_analisis_T = rf"C:\MeteoRBD.v1.0.0\revisión\{num_estacion}-ema\Temperatura"

        # Verifica si existe la ruta de análisis
        if os.path.exists(ruta_analisis_T):
            # Tomar el primer archivo encontrado
            nombre_archivo = next((item for item in os.listdir(ruta_analisis_T) if "T.h.bd3" in item), None)

            if nombre_archivo:
                # Crear el nuevo nombre de archivo
                nombre_carpeta_T = os.path.splitext(nombre_archivo)[0].replace("T.h.bd3", "T.bd.ema")

                ruta_respaldo_T = rf"\\respaldos\CC-RBD\{num_estacion}\Temperatura"

                # Crear la subcarpeta con el nombre_carpeta_T dentro de 'ruta_respaldo_T'
                ruta_respaldo = os.path.join(ruta_respaldo_T, nombre_carpeta_T)
                ...
                for item in os.listdir(ruta_analisis_T):
                    ruta_origen = os.path.join(ruta_analisis_T, item)
                    ruta_destino = os.path.join(ruta_respaldo, item)
                    shutil.move(ruta_origen, ruta_destino)

                print('----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------')
                print(f"Se han guardado los archivos de la ruta {ruta_analisis_T} en la ruta: {ruta_respaldo_T}\{nombre_carpeta_T}")
            else:
                print('----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------')
                print(f"No se encontro el archivo T.h.bd3 en la ruta: {ruta_analisis_T}, no es posible respaldar los archivos.")
        else:
            print('----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------')
            print(f"No se encontraron archivos en la ruta: {ruta_analisis_T}.")
    else:
        print('----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------')
